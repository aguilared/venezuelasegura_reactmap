...
class App extends Component {
  ...
  getData() {
    setTimeout(() => {
      ...
      Object.keys(this.state.homiPorParroquiasAno).map((key) => {
        this.setState.tothomiCachamay = this.state.homiPorParroquiasAno[key].ano_parr_cacha;
        this.setState.tothomiChirica = this.state.homiPorParroquiasAno[key].ano_parr_chi;
        this.setState.tothomiDallaCosta = this.state.homiPorParroquiasAno[key].ano_parr_dalla;
        this.setState.tothomiOncedeAbril = this.state.homiPorParroquiasAno[key].ano_parr_once;
        this.setState.tothomiPozoVerde = this.state.homiPorParroquiasAno[key].ano_parr_pozo;
        this.setState.tothomiSimonBolivar = this.state.homiPorParroquiasAno[key].ano_parr_simon;
        this.setState.tothomiUnare = this.state.homiPorParroquiasAno[key].ano_parr_unare;
        this.setState.tothomiUniversidad = this.state.homiPorParroquiasAno[key].ano_parr_uni;
        this.setState.tothomiVistaAlsol = this.state.homiPorParroquiasAno[key].ano_parr_vista;
        this.setState.tothomiYocoima = this.state.homiPorParroquiasAno[key].ano_parr_yoco;
        // console.log('Homicidios Cachamay = ' + tothomiCachamay);
        return (
          this.state.homiPorParroquiasAno[key].ano_parr_cacha
        );
      });
  ...
  async initialFetch() {
    if (!this.state.homiPorParroquiasAno) return this.setState({ loading: false });
    const homiPorParroquiasAno = await newsApi.getPorParroquiasMes();
    //homiPorParroquiasAno = homiPorParroquiasAno
    //console.log('Homi del mes: '+homiPorParroquiasAno);
    this.setState({
      homiPorParroquiasAno,
      loading: false,

    });
    console.log('Homi del mes: ' +this.state.homiPorParroquiasAno[0]);
  }
